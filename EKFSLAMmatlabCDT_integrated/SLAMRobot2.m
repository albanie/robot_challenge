...
%% First we check to ensure that we are receiving the odometry data
...
%% Loop over incoming sensor readings
...
while true
    ...
    if ~isempty(scan) && ~isempty(wheel_odom)
        ...
        % find poles
        set(0, 'currentfigure', laser_fig);
        clf;
        hold on;
        ShowLaserScan(scan);
        [pole_cartesian, pole_polar] = PoleDetector(scan, 800, 0.1);
        scatter(pole_cartesian(:,1), pole_cartesian(:,2), 20, 'o', 'red') %, 'filled');

        % do data associations
        z = SLAMDataAssociations2(x, pole_polar);

        [x P] = SLAMMeasurement(z, x, P);

        set(0, 'currentfigure',map_fig);
        a = axis;
        clf;
        axis(a);
        hold on;
