...
while true
    ...
    if ~isempty(scan) && ~isempty(wheel_odom)
        ...
        car_measurements = add_wheel_reading(car_measurements, wheel_odom);
        last_deltas = get_last_wheel_reading(car_measurements); 
        % estimated_pose: x-forward, y-right, theta-clockwise
        d_l = last_deltas(1,1);
        d_r = last_deltas(2,1);
        estimated_pose = estimate_pose(car_measurements, d_l, d_r);
        car_measurements = log_pose(car_measurements, estimated_pose);
...
%         [x P] = SLAMMeasurement(z, x, P);
        ...
        hold on;
        logged_pose = car_measurements.logged_poses(1:2, end)
        scatter( logged_pose(1,1), logged_pose(2,1));
        drawnow;
